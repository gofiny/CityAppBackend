...
async def register(server, gp_id: str, username: str) -> dict:
    async with server.pg_pool.acquire() as conn:
        async with conn.transaction():
            user = await db.get_user_info_or_none(conn, gp_id, username)
            if user:
                if user["gp_id"] == gp_id:
                    response = {"is_new_user": False}
                    user = await User.get_by_gp_id(conn, gp_id)
                    game_objects = await db.get_all_game_objects_by_gp_id(conn=conn, gp_id=gp_id)
                ...
                await db.set_game_objects_on_map(conn=conn, objects=[
