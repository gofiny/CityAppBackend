...
async def get_current_action_data(pool: Pool, mo_uuid: str) -> Optional[Record]:
    async with pool.acquire() as conn:
        return await conn.fetchrow(
            "SELECT go.name as pawn_name, pt.way, pt.mo_uuid as target_uuid, pa.name as action_name, " 
...
async def update_pawn_task_time(
    ...
    if calc_time is True:
        start_time = int(time())
        end_time = int(start_time + common_time)
    else:
        start_time = "null"
        end_time = "null"
    ...
    await conn.execute(
        ...
        f"WHERE uuid='{task_uuid}' RETURNING mo_uuid) "
...
async def add_walk_pawn_action(
    ...
    if updating is True:
        await update_pawn_task_time(
            conn=conn,
            task_uuid=pawn_task["uuid"],
            common_time=pawn_task["common_time"]
        )
